...
function detectMultibaggerSignals(
  ...
  if (financials.length < 2) {
    // Still check thesis signals even without financials
  } else {
  ...
  // Management credibility (old commitments system)
  ...
  // ═══════════════════════════════════════
  // THESIS EXECUTION SIGNALS (from promises & snapshots)
  // ═══════════════════════════════════════
  ...
  if (promises.length > 0) {
    ...
    if (resolved >= 2) {
      ...
      if (credRate >= 80) {
        signals.push({ label: `Promise credibility ${credRate}% (${kept}/${resolved})`, type: "bullish" });
      } else if (credRate <= 40) {
        signals.push({ label: `Broken promises ${broken}/${resolved}`, type: "bearish" });
      } else {
        signals.push({ label: `Promise track record ${credRate}%`, type: "warning" });
      }
    }

    // Overdue promises (target_deadline has passed)
    const overdue = promises.filter((p: any) => {
      if (p.status !== "pending" || !p.target_deadline) return false;
      // Simple check: if target quarter contains a past FY year
      return false; // We'll check via snapshots instead
    });
  ...
  // Red flag accumulation from snapshots
  if (snapshots.length > 0) {
    const latestSnap = snapshots[0]; // sorted desc
    ...
    // QoQ improvement check (if 2+ snapshots)
